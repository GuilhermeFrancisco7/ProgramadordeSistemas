...
namespace FormularioSimples
{
    public partial class frmFormularioSimples : Form
    {
        ...
        private void CadastrarNovoCliente()
        {
            try
            {
                using (MySqlConnection conexao = new MySqlConnection(data_source))
                {
                    ...
                    cmd.Connection = conexao; // Corrigido: usando a variável local 'conexao'

                    // Query corrigida (adicionado @nomesocial e fechado parêntese)
                    cmd.CommandText = @"INSERT INTO dados (
                                numerocadastro, nomecompleto,nomesocial,  datanascimento, cidade,  genero)" +
                                "VALUES (  @codigo,   @nomecompleto,  @nomesocial,  @datanascimento,  @cidade,  @genero )";

                    // Adiciona parâmetros (corrigido para usar .Text)
                    cmd.Parameters.AddWithValue("@codigo", txtNumeroCadastro.Text.Trim());
        ...
        private void btnDeletar_Click(object sender, EventArgs e)
        {
            try
            {
                DialogResult opcaoescolhida = MessageBox.Show("Deseja EXCLUIR esse cliente?",
                                                          "Alerta de EXCLUSÂO",
                ...
                if (opcaoescolhida == DialogResult.Yes)
                {
                    ...
                    cmd.CommandText = "DELETE FROM dados WHERE numerocadastro = @codigo";

                    cmd.Parameters.AddWithValue("@codigo", txtNumeroCadastro.Text);
